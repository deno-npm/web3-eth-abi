    ...
    if (typeof type === "string" && type.includes("tuple")) {
      ...
      const modifyParams = (coder, param) => {
        if (coder.name === "array") {
          return param.map((p) =>
            modifyParams(
              ethersAbiCoder._getCoder(
                ParamType.from(coder.type.replace("[]", "")),
              ),
              p,
            )
          );
...
ABICoder.prototype.isSimplifiedStructFormat = function (type) {
  return typeof type === "object" && typeof type.components === "undefined" &&
...
ABICoder.prototype.decodeLog = function (inputs, data, topics) {
  ...
  inputs.forEach(function (input, i) {
    if (input.indexed) {
      indexedParams[i] =
        (["bool", "int", "uint", "address", "fixed", "ufixed"].find(
            function (staticType) {
  ...
  var notIndexedParams = (nonIndexedData)
    ? this.decodeParameters(notIndexedInputs, nonIndexedData)
    : [];
